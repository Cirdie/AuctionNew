...
class BidRepository extends BaseCrudRepository implements BidRepositoryInterface
{
     ...
     * @return void
     */
    public function bid(string $adSlug, User $user, array $data): void
{
    // Load ad
    ...
    // Validate correct minimum bid
    if ($data['amount'] < $nextBid) {
        throw new BidException(
            "Your bid must be at least " . money($nextBid),
            $ad->slug
        );
    }
...
public function getUserBid(string $bid, User $user): \App\Models\Bid
{
    return $this->model
        ->with([
            'ad:id,title,slug,price,status,started_at,expired_at,seller_name,seller_email,seller_mobile,seller_address',
            'ad.user:id,name,avatar,username',
            'payment:id,bid_id,amount,txn_id,status,created_at,payment_method,proof_image'
        ])
        ->where('id', $bid)
        ->where('user_id', $user->id)
        ->firstOr(function () {
